		...
		$(document).ready(function(){
			...
			$.ajax({
				  ...
				  success : function(response) {
					  ...
					  let dicLength = Object.keys(response).length
					  let keyVal = Object.entries(response)
					  //console.log(keyVal[0][1][0][0])
					  
					  console.log(response)
					  //console.log(response["정직한 후보2"].length)
					  //console.log(response[key[0]][0][0])
/* 					  
	 						$(".theater-tit").append('<p>'+key[0]+'</p>')
							$(".theater-type").append('<p class="theater-name">'+'N관'+'</p>'+'<p class="chair">'+'N석'+'</p>')
							$(".time-list-table tbody").append(
							'<tr>'+
							'<td><div class="td-ab"><div class="txt-center"><p class="time">'+response[key[0]][0][0]+'</p>'+'<p class="chair">'+response[key[0]][0][1]+'</p></td>'+
							'</tr>'
							)
							
							'<div class="theater-time"><div class="theater-type-area">'+'2D'+'</div>'+
								'<div class="theater-time-box"><table class="time-list-table"><tbody>'+
								'<tr>'+
								'<td><div class="td-ab"><div class="txt-center"><p class="time">'+response[key[0]][0][0]+'</p>'+'<p class="chair">'+response[key[0]][0][1]+'</p></td>'+
						...
						for(var i = 0; i < dicLength; i++) {
							/* console.log("dicLength : " + dicLength) */
							$(".theater-list").append(
								'<div class="theater-tit">'+key[i]+'</div>'+
								'<div class="theater-type-box"><div class="theater-type"><p class="theater-name">'+'N관'+'</p>'+'<p class="chair">'+'N석'+'</p></div>'
							)
								
							
							//console.log(keyVal[i][1])
							//console.log(keyVal[i][1].length)
							
							for(var j = 0; j < keyVal[i][1].length; j++) {
								var movieNm = keyVal[i]
								var theabExpoNm = keyVal[i][1][j][0]
								console.log(theabExpoNm)
							
								//arr[arr.length] = theabExpoNm;
								//console.log(arr)
								
								let str = '<p class="time">'+theabExpoNm+'</p>'
								$(".theater-list").append(str)
										...
										+'<div class="theater-time-box">'
											+'<table class="time-list-table">'
												+'<tbody>'
													+'<tr>'
														+'<td>'
															+'<div class="td-ab"><div class="txt-center"><p class="time">${theabExpoNm}</p></div></div>'
														+'</td>'
													+'</tr>'
												+'</tbody>'
											+'</table>'
